...
#sample ZIPs for faster testing
test_zips = ["90001", "90002", "90003"]

def test_la_income_zips():
    df = get_la_income_zips(test_zips)
    
    #checking the number of rows
    assert df.shape[0] == len(test_zips), \
        "Number of rows does not match number of test ZIPs"
    
    #required columns
    expected_columns = ["NAME", "B19013_001E", "state", "zip_code"]
    for col in expected_columns:
        assert col in df.columns, f"Missing column: {col}"
    
    #checking all ZIPs returned
    missing_zips = [z for z in test_zips if z not in df["zip_code"].values]
    assert not missing_zips, f"Missing ZIPs in DataFrame: {missing_zips}"
    
    print("Okay: ACS LA ZIP income data retrieved correctly")
